...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.entities.background','game.entities.ground','game.entities.rock','game.entities.tap','game.entities.player').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/UI/KenPixelBlocks.png'),gravity:0,rockArray:[],Player:null,tap:null,PlayerScore:0,highScore:0,IsStarted:false,IsPlayerAlive:true,init:function(){ig.input.bind(ig.KEY.MOUSE1,'jump');ig.input.bind(ig.KEY.SPACE,'jump');ig.input.bindTouch('#canvas','jump');ig.input.bind(ig.KEY.R,'Restart');this.spawnEntity(EntityBackground,0,0);this.Player=this.spawnEntity(EntityPlayer,50,ig.system.height/2-(73/2));this.spawnEntity(EntityGround,0,ig.system.height-41);this.spawnEntity(EntityGround,ig.system.width+8,ig.system.height-41);this.tap=this.spawnEntity(EntityTap,this.Player.pos.x+100,this.Player.pos.y+15);this.rockArray.push(this.spawnEntity(EntityRock,300,ig.system.height-179,"Up"));this.rockArray.push(this.spawnEntity(EntityRock,700,-60,"Down"));},update:function(){this.parent();this.sortEntitiesDeferred();if(this.IsStarted===true){this.gravity=1000;this.tap.kill();}else{if(ig.input.pressed('jump')){this.IsStarted=true;}}
