...
ig.baked=true;ig.module('game.entities.background').requires('impact.entity').defines(function(){EntityBackground=ig.Entity.extend({size:{x:800,y:480},animSheet:new ig.AnimationSheet('media/background.png',800,480),gravityFactor:0,zIndex:-3,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[0]);}});});

// lib/game/entities/ground.js
ig.baked=true;ig.module('game.entities.ground').requires('impact.entity').defines(function(){EntityGround=ig.Entity.extend({size:{x:808,y:40},offset:{x:0,y:30},animSheet:new ig.AnimationSheet('media/groundDirt.png',808,71),gravityFactor:0,zIndex:-1,collides:ig.Entity.COLLIDES.NEVER,type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[0]);},update:function(){this.parent();if(ig.game.IsPlayerAlive===true&&ig.game.IsStarted===true){this.pos.x-=1;}
if(this.pos.x<-ig.system.width){this.pos.x=ig.system.width+8;}}});});

// lib/game/entities/rock.js
ig.baked=true;ig.module('game.entities.rock').requires('impact.entity').defines(function(){EntityRock=ig.Entity.extend({size:{x:30,y:215},offset:{x:50,y:0},animRock:new ig.AnimationSheet('media/rock.png',108,239),animRockDown:new ig.AnimationSheet('media/rockDown.png',108,239),animSheet:null,gravityFactor:0,zIndex:-2,collides:ig.Entity.COLLIDES.NEVER,type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,init:function(x,y,RockType){this.parent(x,y);if(RockType==="Up"){this.animSheet=this.animRock;this.offset.y=24;}else if(RockType==="Down"){this.animSheet=this.animRockDown;}
this.addAnim('idle',1,[0]);},update:function(){this.parent();if(this.pos.x===ig.game.Player.pos.x){ig.game.PlayerScore+=1;}}});});

// lib/game/entities/tap.js
ig.baked=true;ig.module('game.entities.tap').requires('impact.entity').defines(function(){EntityTap=ig.Entity.extend({size:{x:59,y:49},animSheet:new ig.AnimationSheet('media/UI/tap_spritesheet.png',59,59),gravityFactor:0,zIndex:-3,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.6,[0,1]);}});});

// lib/game/entities/player.js
ig.baked=true;ig.module('game.entities.player').requires('impact.entity').defines(function(){EntityPlayer=ig.Entity.extend({size:{x:88,y:73},animSheet:new ig.AnimationSheet('media/Planes/planeRed.png',88,73),maxVel:{x:0,y:3000},collides:ig.Entity.COLLIDES.NEVER,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.25,[0,1,2]);},update:function(){this.parent();if(ig.input.pressed('jump')){this.vel.y=-450;}
if(this.pos.y<0){this.pos.y=0;this.vel.y=0;}},check:function(other){ig.game.IsPlayerAlive=false;this.kill();}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.entities.background','game.entities.ground','game.entities.rock','game.entities.tap','game.entities.player').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/UI/KenPixelBlocks.png'),gravity:0,rockArray:[],Player:null,tap:null,PlayerScore:0,highScore:0,IsStarted:false,IsPlayerAlive:true,init:function(){ig.input.bind(ig.KEY.MOUSE1,'jump');ig.input.bind(ig.KEY.SPACE,'jump');ig.input.bindTouch('#canvas','jump');ig.input.bind(ig.KEY.MOUSE1,'restart');ig.input.bind(ig.KEY.SPACE,'restart');this.spawnEntity(EntityBackground,0,0);this.Player=this.spawnEntity(EntityPlayer,50,ig.system.height/2-(73/2));this.spawnEntity(EntityGround,0,ig.system.height-41);this.spawnEntity(EntityGround,ig.system.width+8,ig.system.height-41);this.tap=this.spawnEntity(EntityTap,this.Player.pos.x+100,this.Player.pos.y+15);this.rockArray.push(this.spawnEntity(EntityRock,300,ig.system.height-179,"Up"));this.rockArray.push(this.spawnEntity(EntityRock,700,-60,"Down"));},update:function(){this.parent();this.sortEntitiesDeferred();if(this.IsStarted===true){this.gravity=1000;this.tap.kill();}else{if(ig.input.pressed('jump')){this.IsStarted=true;}}
if(this.rockArray.length<3){this.rockArray.push(this.spawnEntity(EntityRock,1100,ig.system.height-179,"Up"));this.rockArray.push(this.spawnEntity(EntityRock,1500,-60,"Down"));}
if(ig.input.state('Restart')&&this.IsPlayerAlive===false){ig.system.setGame(MyGame);}
if(this.IsPlayerAlive===true&&this.IsStarted===true){for(i=0;i<this.rockArray.length;i++){this.rockArray[i].pos.x-=1;if(this.rockArray[i].pos.x<-150){this.rockArray[i].kill();this.rockArray.shift();}}}},draw:function(){this.parent();this.font.draw(this.PlayerScore,ig.system.width/2,20,ig.Font.ALIGN.CENTER);if(this.IsStarted===false){this.font.draw("Art By Kenney.nl",20,ig.system.height-30,ig.Font.ALIGN.LEFT);this.font.draw("@Jack_Hodgkiss",ig.system.width-20,ig.system.height-30,ig.Font.ALIGN.RIGHT);}
if(this.IsStarted===true&&this.IsPlayerAlive===false){this.font.draw("Press 'R' To Restart",ig.system.width/2,ig.system.height/2-20,ig.Font.ALIGN.CENTER);}}});ig.main('#canvas',MyGame,60,800,480,1);});